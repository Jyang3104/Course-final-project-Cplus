...
namespace sdds
{
	...
	public:
		...
		int getDays()const;//returns number of days passed since the date 0001/1/1 
		std::istream& read(std::istream& is = std::cin);//read date from console
		std::ostream& write(std::ostream& os = std::cout) const;//display date info
		operator bool() const;
	};
